...
## There are 380 genes with consistent direction across 2 studies
...
# Load the data
train_data_split1 <- read.csv(here("data", "Train_Dataset_70.csv"), row.names = 1)
test_data_split1 <- read.csv(here("data", "Test_Dataset_30.csv"), row.names = 1)
...
png(filename = here("results","LASSO_AUC_and_Accuracy_plot.png"),
    height = 6, width = 8,
    units = 'in', res = 300)
# Base plot: AUC
...
png(filename = here("results","Predictor_genes_roc_train_data.png"), 
...
for (gene in auc_results_train$Gene) {
  ...
  png(filename = here("results", paste0("Train_ROC_", gene, ".png")),
...
png(filename = here("results", "Predictor_genes_roc_test_data.png"), 
    width = 10, height = 8, units = "in", res = 300)
...
for (gene in auc_results_test$Gene) {
  ...
  png(filename = here("results", paste0("Test_ROC_", gene, ".png")),
      width = 8, height = 8, units = "in", res = 300)
...
write.csv(combo_results, here("results", "LASSO_combinations_AUC_results.csv"), row.names = FALSE)
...
output_dir <- here("results","Combination_Top10_ROC_Plots")
